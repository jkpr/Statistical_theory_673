...
# Problem 2
# We take the log likelihood, we need expressions for 
# f(Y_i, \theta) = pr(Y_i | \theta) = 
# Y_i
# E_\theta[Y_i]
# and their logs
...
optimOut <- suppressWarnings(optim(par = c(theta1, theta2), logLikelihood, 
                                   dat=dat, control = list(fnscale=-1), 
                                   hessian = TRUE))
# Optimized theta1 and theta2
mleTheta1 <- optimOut$par[1]
mleTheta2 <- optimOut$par[2]
@


The optimal values are $\hat{\theta_1}=\Sexpr{mleTheta1}$ and $\hat{\theta_2}=\Sexpr{mleTheta2}$.
\end{proof}
...
<<>>=
# http://www.math.northwestern.edu/~clark/285/2006-07/handouts/pos-def.pdf
# A symmetric matrix A is negative-definite if and only if all its eigenvalues
# are negative
# 1. test if hessian is symmetric
hess <- optimOut$hessian
testSym <- isSymmetric(hess)
testSym
# 2. test if eigenvalues are negative
vals <- eigen(hess)$values
# Test: the number of nonnegative eigenvalues is 0
negEig <- sum(vals >= 0) == 0
negEig
@
...
By the calculations, $\hat{x} = \Sexpr{percentile95}$.
\item
...
t1t1 <- cov(mle$t1, mle$t1)
t1t2 <- cov(mle$t1, mle$t2)
t2t2 <- cov(mle$t2, mle$t2)
@
...
We get that an unbiased estimate of $g(\mu)$ is \Sexpr{unB}
\end{proof}
